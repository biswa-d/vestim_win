...
class ContinuousTestingService:
    ...
    def _denormalize_values(self, y_pred, y_true, target_col):
        ...
        try:
            ...
            # Check if scaler has feature_names_in_ attribute (newer sklearn versions)
            if hasattr(self.scaler, 'feature_names_in_') and self.scaler.feature_names_in_ is not None:
                try:
                    ...
                    print(f"WARNING: Target column '{target_col}' not found in scaler features: {list(self.scaler.feature_names_in_)}")
                    return y_pred, y_true
            else:
                # Fallback: assume target is the last column if scaler has multiple features
                if hasattr(self.scaler, 'data_min_') and len(self.scaler.data_min_) > 1:
                    target_idx = len(self.scaler.data_min_) - 1  # Last column
                    print(f"WARNING: No feature names available, assuming target is at index {target_idx}")
                else:
                    target_idx = 0  # Single feature scaler
                    print(f"Single feature scaler detected, using index 0")
            
            # Verify the index is valid
            if hasattr(self.scaler, 'data_min_') and target_idx >= len(self.scaler.data_min_):
                print(f"ERROR: Target index {target_idx} out of bounds for scaler with {len(self.scaler.data_min_)} features")
                return y_pred, y_true
            ...
            print(f"Denormalization parameters for {target_col} (index {target_idx}):")
